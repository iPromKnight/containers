...
if [ $? -eq 0 ]; then
    ...
    items=(1 "Run everything now!"
        ...
        6 "Run playlists now"
        7 "Run metadata now"
        8 "Run on schedule (6AM UTC)"
        )

    while choice=$(dialog --title "$TITLE" \
                    --menu "Pick your Kometa task:" 20 40 8 "${items[@]}" \
                    2>&1 >/dev/tty)
        do
        case $choice in
            ...
            8) KOMETA_TIMES=06:00 python3 kometa.py ;;              
    ...
    echo "Timeout reached, running KOMETA as on schedule (${KOMETA_TIMES:-06:00})..."
    KOMETA_TIMES=${KOMETA_TIMES:-06:00} python3 kometa.py
fi